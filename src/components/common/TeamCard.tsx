...
const TeamCard = ({ image, name, position, description, className = '' }: TeamCardProps) => {
  const [isFlipped, setIsFlipped] = useState(false)

  return (
    <div 
      className={`relative w-64 h-80 perspective-1000 ${className}`}
      ...
      >
        ...
        <div className="absolute inset-0 w-full h-full backface-hidden bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-lg">
          ...
          <div className="h-1/4 p-4 flex flex-col justify-center">
            ...
            <p className="text-sm text-primary opacity-70 text-center mt-1">
              {position}
            </p>
          </div>
        </div>
        ...
        <div className="absolute inset-0 w-full h-full backface-hidden rotate-y-180 bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-lg">
          <div className="h-full p-6 flex flex-col justify-center">
            ...
            <p className="text-sm text-primary opacity-70 text-center mb-4">
              {position}
            </p>
            <p className="text-sm text-primary opacity-80 text-center leading-relaxed">
              {description}
            </p>
          </div>
        </div>
